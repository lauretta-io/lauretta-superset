...
try:
    ...
    for dashboard in dashboards:
        ...
        # Pre-validate connection count without extraction
        if connections:
            yaml_count = count_yaml_files_in_zip(dashboard_path)
            conn_count = len(connections)
            
            if conn_count > yaml_count:
                print(f"✗ ERROR: Too many connections in config!")
                print(f"  Config has {conn_count} connections but ZIP has only {yaml_count} YAML files")
                fail_count += 1
                continue
            elif conn_count < yaml_count:
                print(f"⚠️  WARNING: Config has {conn_count} connections but ZIP has {yaml_count} YAML files")
                print(f"  Only first {conn_count} connections will be updated")
