...
# INCLUDE_FIREFOX is true. Superset 6.0 flipped that default from true to false
# (UPDATING.md, PR 34258) to keep the lean layer lean, so no image carries a browser
# unless a compose file asks for one -- docker-compose-secure.yml pins
# INCLUDE_CHROMIUM=true for exactly this reason. So the answer differs per mode, and
# is a property of the image rather than of the deployment.
try:
...
FEATURE_FLAGS = {
    "ALERT_REPORTS": True,
    # Enabled by upstream's 6.x docker config; carried forward.
...
if os.getenv("SUPERSET_SECURE_MODE", "").strip().lower() in {"1", "true", "yes"}:
    ...
    from superset_config_secure import *  # noqa: F403

    logger.info(
        "Loaded hardened secure configuration at [%s]", superset_config_secure.__file__
    )
