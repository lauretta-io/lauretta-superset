...
if [[ "$DATABASE_DIALECT" == postgres* ]] && [ "$(whoami)" = "root" ] && [ "$1" != "worker" ] && [ "$1" != "beat" ] && \
   ! python -c "import psycopg2" > /dev/null 2>&1; then
    # older images may not have the postgres dev requirements installed
    #
    # Skipped when the driver is already importable, which it is in every image
    # this repo builds: the `dev` stage installs .[postgres] (Dockerfile) and
    # `production` installs .[postgres,thumbnails]. Without the guard this runs
    # on any root container -- modes 1 and 2, but not 3, which runs as
    # `superset` -- and `uv pip install -e .[postgres]` rebuilds apache-superset
    # from /app, so uv fetches setuptools and wheel from PyPI for
    # build-system.requires. On an air-gapped host that is a hard failure in
    # superset-init, which calls this script before running any migration:
    #
